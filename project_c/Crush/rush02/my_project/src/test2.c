...
char *ft_getnb(int fd)
{
    int i = 0;
    char c[1];
    char *str = malloc(MAX_STR_SIZE); //  information
    if (!str)
    {
...
t_list *process(const char *file) {
    ...
    tab = malloc(sizeof(int) * MAX_ENTRIES); // important
    if (!tab) {
    ...
    i = 0;
    while (i < MAX_ENTRIES && read(fd, c, 1) > 0) { // continue to loop until the the file descriptor read one byte into 'c' 
        // this stage it's important because the upcoming function getnbr() will read the number  starting fromm this position
        lseek(fd, -1, SEEK_CUR);  // function calls moves the file pointer one byte backward from its current position.
        char *nb_str = ft_getnb(fd);  // it's called to read the sequence of digits from the file, starting frommt the current position. 
        tab[i].nb = ft_atoi(nb_str); // the digits are returned as a pointer and hold in the tab[i].nb
        free(nb_str); // free and no longer needed.

        read(fd, c, 1);
        if (c[0] == ':')  // after reading the number, the code read the next character to see if it's a colon : 
            read(fd, c, 1); // read the next character  (skipping the column)
        while (c[0] == ' ') // the code sips nay spaces following the colon, poisitionning the file p9ointer at the start of the valuue.
            read(fd, c, 1);
        
        tmp = ft_getval(fd, c); // read the value  associated with the numbers until it enncounters a new line
        tab[i].val = ft_strdup(tmp); // duplicates the string and assign it to `tab[i].val`
        free(tmp); // the memory allocated to tmp is freed because already exist a new copy of the string.
        i++;
    }
    close(fd);
    return tab;
